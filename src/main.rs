...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  if args.next().is_some() { panic!("expected exactly two arguments") }
  ...
  let mut framevec = Vec::with_capacity(frame_count);
